  ...
  redirigir(response: any) {
    if (response.resultado.state == 200 && response.resultado.token != '') {
      ...
      this.localStorage.setItem(
        'tipoDocumento',
        response.resultado.tipoDocumento
      );
      ...
      this.mensaje.MostrarMensaje(
        ...
        'Bienvenido:' +
          this.login.usuario +
          ' ' +
          this.localStorage.getItem('cuenta') +
          ' ' +
          this.localStorage.getItem('nombres') +
          ' ' +
          this.localStorage.getItem('ciudad')
      );
      this.ui.ocultarNav();
      this.router.navigate(['principal']);
    } else if (
      response.resultado.state == 200 &&
